...
export default function Payment() {
  ...
  const composeMessage = () => {
    ...
    for (const it of cartList) {
      ...
      const url = `${window.location.origin}/category/${catSlug}/${prodSlug}`;
    ...
    if (address && typeof address === 'object') {
      ...
      if (fullName) contact.push(`Name: ${fullName}`);
  ...
  const onPay = () => {
    try {
      ...
      setTimeout(() => { try { if (!win || win.closed) window.location.href = fb; } catch { window.location.href = fb; } }, 350);
    } catch {
      try {
        const message = composeMessage();
        window.location.href = `https://wa.me/919933778870?text=${encodeURIComponent(message)}`;
      } catch {}
    }
  };
